    ...
    def predict(self, image, conf_threshold: float = 0.15, return_image: bool = False):
        ...
        if results[0].boxes is not None:
            ...
            for box in results[0].boxes:
                ...
                image_area = image.size[0] * image.size[1] if hasattr(image, 'size') else results[0].orig_shape[0] * results[0].orig_shape[1]
                ...
                # Draw bounding box if image requested
                if return_image:
            ...
            # Convert annotated image to base64 if requested
            if return_image:
                buffer = io.BytesIO()
